...
def get_arg_parser():
    ...
    ap = argparse.ArgumentParser(
        description='Run a wxpyit-ready python console.')
...
def shell_entry():
    ...
    if args.bot:
        ...
        try:
            ...
            for name in args.bot:
                if not re.match(r'\w+$', name):
                    continue
                cache_path = 'wxpyit_{}.pkl'.format(name) if args.cache else None
                bots[name] = wxpyit.Bot(cache_path=cache_path, console_qr=args.console_qr)
        except KeyboardInterrupt:
            return
        ...
        module_members = dict(inspect.getmembers(wxpyit))
