...
def get_download_fn(core, url, msgId):
    def download_fn(downloadDir=None):
        ...
        headers = config.FAKE_HEADERS | { 'User-Agent' : config.USER_AGENT }
        ...
        if m['MsgType'] == 1: # words
            ...
            def download_video(videoDir=None):
                ...
                params = {
                    'msgid': msgId,
                    'skey': core.loginInfo['skey'],}
                headers = config.FAKE_HEADERS | {'Range': 'bytes=0-', 'User-Agent' : config.USER_AGENT }
...
def send_raw_msg(self, msgType, content, toUserName):
    ...
    data = {
        ...
        'Scene': 0, }
    headers = config.FAKE_HEADERS | { 'ContentType': 'application/json; charset=UTF-8', 'User-Agent' : config.USER_AGENT }
